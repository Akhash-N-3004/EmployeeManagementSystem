...
public class JobDepartmentService {
  ...
  public JobDepartment updateJobDepartment(Long id, JobDepartment jobDepartment) {
	  JobDepartment jobdep = jobDepartmentRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Employee not found"));
		if(jobDepartment.hasDeptName()) {
			jobdep.setDeptName(jobDepartment.getDeptName());
		}
		return jobDepartmentRepository.save(jobdep);
	}
