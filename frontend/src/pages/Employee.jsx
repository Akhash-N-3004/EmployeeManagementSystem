...
import { addAbsence, getAbsences, updateEmp, updatePwd } from "@/lib/api";
...
const Employee = () => {
  ...
  const [openForPwd, setOpenForPwd] = useState(false);
  ...
  const pwdMutation = useMutation(updatePwd, {
    onError: () => {
      toast.error("olunga adi da punda!");
    },
    onSuccess: () => {
      toast.success("seri da punda!");
      setOpenForPwd(false);
    },
  });
  ...
  const setPwdToUseState = ({ target: { value } }) => {
    setPwd((prev) => (value));
  }
  ...
  return (
    <>
      ...
      <Dialog.Root open={openForPwd} onOpenChange={setOpenForPwd}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 bg-blackA9 data-[state=open]:animate-overlayShow" />
          <Dialog.Content className="fixed left-[50%] top-[50%] max-h-[85vh] w-[90vw] max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none data-[state=open]:animate-contentShow">
            <Dialog.Title className="m-0 text-xl font-medium text-mauve12">
        ...
        <div className="flex-1 rounded-md border border-slate-300 p-5">
          <div className="mb-5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button size="xs" variant="secondary" color="gray"  onClick={setOpenForPwd}>
                Update password
              </Button>
              <Button size="xs" variant="secondary" color="gray" onClick={() => openInNewTab(`http://localhost:8080/employees/${user?.id}/certificate`)}>
                Request certificate
              </Button>
